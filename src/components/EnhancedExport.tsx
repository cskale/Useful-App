...
export const EnhancedExport: React.FC<EnhancedExportProps> = ({ data }) => {
  ...
  const generatePDFReport = async () => {
    try {
      const { data: pdf, error } = await supabase.functions.invoke<ArrayBuffer>(
        'generate-pdf-report',
        {
          body: {
            ...
            assessmentType: data.dealership?.assessmentType || 'Assessment',
          },
      ...
      const fileName = `${(data.dealership?.name || 'Dealer').replace(/\s+/g, '_')}_${
        data.dealership?.assessmentType || 'Assessment'
